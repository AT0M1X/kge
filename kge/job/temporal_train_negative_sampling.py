...
from kge.util.temporal_sampler import TemporalKgeSampler
...
class TemporalTrainingJobNegativeSampling(TrainingJob):
    def __init__(
        ...
        self._sampler = TemporalKgeSampler.create(
            config, "temporal_negative_sampling", dataset
        )
        self.type_str = "temporal_negative_sampling"

        if self.__class__ == TemporalTrainingJobNegativeSampling:
    ...
    def _prepare(self):
        ...
        self._implementation = self.config.check(
            "temporal_negative_sampling.implementation",
            ["triple"],
        )
    ...
    def _get_collate_fun(self):
        def collate(batch):
            ...
            negative_samples = [
                self._sampler.sample(triples, slot) for slot in [S, P, O, T]
            ]
